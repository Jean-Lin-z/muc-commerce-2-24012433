...
def load_dashboard_data(base_dir: Path, selected_category: str = "全部") -> dict:
    ...
    categories = ["全部", *category_df["PreferredOrderCat"].tolist()]
    table_df = category_df.copy()

    # ===================== TODO 3-1 完成：按选中品类筛选表格数据 =====================
    if selected_category != "全部":
        table_df = table_df[table_df["PreferredOrderCat"] == selected_category]

    table_df = table_df.rename(
        columns={
            "PreferredOrderCat": "偏好品类",
            "用户数": "用户数",
            "流失率": "流失率",
            "平均订单数": "平均订单数",
        }
