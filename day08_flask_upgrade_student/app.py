...
def metrics_api():
    ...
    raw_data = load_metric_api_data(BASE_DIR)
    if hasattr(raw_data, "to_dict"):
        serial_data = raw_data.to_dict("records")
    else:
        serial_data = raw_data
    return jsonify({"ok": True, "metrics": serial_data})
...
def categories_api():
    ...
    raw_rows = load_category_api_data(BASE_DIR, category)
    if hasattr(raw_rows, "to_dict"):
        serial_rows = raw_rows.to_dict("records")
    else:
        serial_rows = raw_rows
    return jsonify({"ok": True, "category": category, "rows": serial_rows})
