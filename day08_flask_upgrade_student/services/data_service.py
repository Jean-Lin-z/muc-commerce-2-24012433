...
def load_metric_api_data(base_dir: Path) -> list[dict]:
    ...
    if hasattr(raw_metrics, "to_dict"):
        serializable_metrics = raw_metrics.to_dict("records")
    else:
        serializable_metrics = raw_metrics
...
def load_category_api_data(base_dir: Path, selected_category: str = "全部") -> list[dict]:
    ...
    raw_rows = data["category_rows"]
    if hasattr(raw_rows, "to_dict"):
        serializable_rows = raw_rows.to_dict("records")
    else:
        serializable_rows = raw_rows
